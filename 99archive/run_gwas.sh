...
if [[ -z "$phenotype_file" ]]; then
  ...
  phenotype_file="input/phenotype.csv"
fi
...
mkdir -p ${workingdirectory}/{raw_files,phenotype_file,freq_files,fasta_files,blasted_seqs,blastmeta,codon_align,results,for_alignment_aa,for_alignment_nt,alignment_aa,alignment_nt,alignment_basefreq,pca,gwas_data}

# copy raw files, phenotype data, and reference sequence files in working directory
cp $raw_files/*  ${workingdirectory}/raw_files/
cp $phenotype_file  ${workingdirectory}/phenotype_file/phenotype_file.csv
...
# Extract regions from each sequence with Blast, generate codon alignments, and calculate apd for pol, gag, and env 
